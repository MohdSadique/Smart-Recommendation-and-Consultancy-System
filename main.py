...
def restType():
    if request.method == 'POST':
        ...
        datas=dfrt.sort_values(['url'],ascending=False).groupby(['rest_type'],as_index=False).apply(lambda x : x.sort_values(by="url",ascending=False).head(5))['url'].reset_index().rename(columns={'url':'count'})
...
def onCuisine():
    if request.method == 'POST':
        ...
        data = pd.DataFrame(df[df['cuisines']==parameter].groupby(['name'],as_index=False)['url'].agg('count'))
...
def consultancy():
    if request.method == 'POST':
        ...
        if parameter == 'Location Vs Cuisines':
            ...
            data=df_1.sort_values(['url'],ascending=False).groupby(['location'],as_index=False).apply(lambda x : x.sort_values(by="url",ascending=False).head(100))['url'].reset_index().rename(columns={'url':'count'})
            if Location == Cuisine == 'All':
                ...
                data = data[(data['location'] == Location) & (data['cuisines'] == Cuisine)]
            ...
            sns.scatterplot(x="rate",y='approx_cost(for two people)',data=cost_dist)
...
def restaurantDensity():
    if request.method == 'POST':
        ...
        if parameter == 'Restaurant Vs Location':
            ...
            locations['Name']=locations['Name'].apply(lambda x: "Pune " + str(x))
            lat_lon=[]
            ...
            locations["Name"]=locations['Name'].apply(lambda x :  x.replace("Pune","")[1:])
...
if __name__ == '__main__':
    ...
    cost_dist = cost_dist[['rate','approx_cost(for two people)','location', 'name','rest_type']]
    cost_dist['rate']=cost_dist['rate'].apply(lambda x : float(x if (x != '-') else np.nan ))
    ...
    cost_dist['approx_cost(for two people)']=cost_dist['approx_cost(for two people)'].apply(lambda x: (x.replace(',','')))
